...
public class JProducerSerial extends Thread {
	...
	public Properties configure() {
		...
		props.put("value.serializer", "com.ling.kafka.produce.serialization.JSalarySeralizer");// 自定义序列化值
	...
	public void run() {
		while(true){
			...
			jss.setId(UUID.randomUUID().toString().substring(0,2));
			jss.setSalary(UUID.randomUUID().toString());

			producer.send(new ProducerRecord<String, JSalarySerial>("a1", "key", jss), new Callback() {
